...
def nonparametric_profile_omnifold(mc, data_S, iterations, model, verbose=0):

    weights = np.empty(shape=(iterations, 3, len(mc)))
    # shape = (iteration, step, event)

    mc_G = mc[:,0]
    mc_S = mc[:,1]

    labels0 = np.zeros(len(mc))
    labels_unknown = np.ones(len(data_S))
    labels_unknown_step2 = np.ones(len(mc_G))

    xvals_1 = np.concatenate((mc_S, data_S))
    yvals_1 = np.concatenate((labels0, labels_unknown))

    xvals_2 = np.concatenate((mc_G, mc_G))
    yvals_2 = np.concatenate((labels0, labels_unknown_step2))

    # initial iterative weights are ones
    weights_pull = np.ones(len(mc_S))
    weights_push = np.ones(len(mc_S))
    ...
    for i in range(iterations):
        ...
        weights_1 = np.concatenate((weights_push * w, np.ones(len(data_S))))
        ...
        weights_pull = weights_push * reweight(mc_S,model)
        weights[i, :1, :] = weights_pull
        ...
        weights_2 = np.concatenate((np.ones(len(mc_G)), weights_pull * w))
        # ones for Gen. (not MC weights), actual weights for (reweighted) Gen.
        ...
        model.fit(X_train_2,
                  ...
                  validation_data=(X_test_2, Y_test_2),
                  verbose=verbose)
        weights_push = reweight(mc_G,model)
        weights[i, 1:2, :] = weights_push
...
def ad_hoc_penalized_profile_omnifold(mc, data_S, iterations, model, w_func, theta_bar, theta0, verbose=0):

    weights = np.empty(shape=(iterations, 4, len(mc)))
    # shape = (iteration, step, event)
...
def penalized_profile_omnifold(mc, data_S, iterations, model, w_func, w_func_derivative, theta_bar, theta0, verbose=0):
    ...
    for i in range(iterations):
        ...
        solution = optimize.root_scalar(theta_func, bracket = [0,3], method='brentq')
        theta = solution.root
        ...
        weights_3 = np.concatenate((np.ones(len(mc_G)), w_next))
        # ones for Gen. (not MC weights), actual weights for (reweighted) Gen.
        X_train_3, X_test_3, Y_train_3, Y_test_3, w_train_3, w_test_3 = train_test_split(xvals_2, yvals_2, weights_3)

        # zip ("hide") the weights with the labels
        Y_train_3 = np.stack((Y_train_3, w_train_3), axis=1)
        Y_test_3 = np.stack((Y_test_3, w_test_3), axis=1)   
        ...
        weights_push = weights_push * 2 / (1 + reweight(mc_G,model))
        weights[i, 1:2, :] = weights_push
        
    return weights


def profile_omnifold_known_nuisance(mc, data_S, iterations, model, w, verbose=0):

    weights = np.empty(shape=(iterations, 2, len(mc)))
    # shape = (iteration, step, event)
...
###############################################################################################################
## old profile omnifold codes

def profile_weighted_binary_crossentropy(theta, theta_bar, w_func):
    def loss(y_true, y_pred):
        weights = tf.cast(tf.gather(y_true, [1], axis=1), tf.float32) # event weights
        y_true = tf.gather(y_true, [0], axis=1) # actual y_true for loss
        idx = tf.gather(y_true, [2], axis=1) # indices for the events
        mc_idx = tf.cast(tf.reshape(tf.boolean_mask(idx, y_true==0), [-1,1]), tf.int32)     # indices for the Monte Carlo events
        w_theta = tf.cast(w_func(theta).cpu(), tf.float32) # evaluate w function at the given theta      
        weights = tf.tensor_scatter_nd_update(weights, mc_idx, tf.math.multiply(tf.gather_nd(weights, mc_idx), tf.gather_nd(w_theta, mc_idx))) # multiply MC weights by the w function evaulated at theta
        ...
        return K.mean(t_loss) + (theta-theta_bar)**2/2
    return loss


def profile_omnifold1(mc, data_S, iterations, model, w_func, theta_bar, verbose=0):
    ...
    for i in range(iterations):
        ...
        weights_1 = np.concatenate((weights_push, np.ones(len(data_S))))
        
        rs = ShuffleSplit(n_splits=1, test_size=.25)
        rs.get_n_splits(xvals_1)
        train_index, test_index = next(rs.split(xvals_1)) 
        X_train_1, X_test_1, Y_train_1, Y_test_1, w_train_1, w_test_1 = xvals_1[train_index], xvals_1[test_index], yvals_1[train_index], yvals_1[test_index], weights_1[train_index], weights_1[test_index]

        # zip ("hide") the weights with the labels
        Y_train_1 = np.stack((Y_train_1, w_train_1, train_index), axis=1)
        Y_test_1 = np.stack((Y_test_1, w_test_1, test_index), axis=1)
        

        model.compile(loss=profile_weighted_binary_crossentropy(theta=model.layers[1].trainable_weights[0].numpy(), theta_bar=theta_bar, w_func=w_func),
                      optimizer='Adam',
                      metrics=['accuracy'])
        model.fit(X_train_1,
                  Y_train_1,
        ...
        print(f'Fitted value of Theta: {model.layers[1].trainable_weights[0].numpy()}')
        ...
        weights_2 = np.concatenate((np.ones(len(mc_G)), weights_pull))
        # ones for Gen. (not MC weights), actual weights for (reweighted) Gen.
        
        rs = ShuffleSplit(n_splits=1, test_size=.25)
        rs.get_n_splits(xvals_2)
        train_index, test_index = next(rs.split(xvals_2)) 
        X_train_2, X_test_2, Y_train_2, Y_test_2, w_train_2, w_test_2 = xvals_2[train_index], xvals_2[test_index], yvals_2[train_index], yvals_2[test_index], weights_2[train_index], weights_2[test_index]

        # zip ("hide") the weights with the labels
        Y_train_2 = np.stack((Y_train_2, w_train_2, train_index), axis=1)
        Y_test_2 = np.stack((Y_test_2, w_test_2, test_index), axis=1)
...
def profile_omnifold2(mc, data_S, iterations, model, w_func, theta_bar, verbose=0):
    ...
    for i in range(iterations):
        # initial weights on the response kernel are determined by the initial theta
        w = w_func(model.layers[1].trainable_weights[0].numpy()).cpu().numpy().flatten()
        ...
        print(f'Value of Theta before update: {model.layers[1].trainable_weights[0].numpy()}')
        
        weights_3 = np.concatenate((weights_push, np.ones(len(data_S))))
        
        rs = ShuffleSplit(n_splits=1, test_size=.25)
        rs.get_n_splits(xvals_1)
        train_index, test_index = next(rs.split(xvals_1)) 
        X_train_3, X_test_3, Y_train_3, Y_test_3, w_train_3, w_test_3 = xvals_1[train_index], xvals_1[test_index], yvals_1[train_index], yvals_1[test_index], weights_1[train_index], weights_1[test_index]

        # zip ("hide") the weights with the labels
        Y_train_3 = np.stack((Y_train_3, w_train_3, train_index), axis=1)
        Y_test_3 = np.stack((Y_test_3, w_test_3, test_index), axis=1)
        ...
        print(f'Updated value of Theta: {model.layers[1].trainable_weights[0].numpy()}')
